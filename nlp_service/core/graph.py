...
def add_user_word_token(user_id: str, word: str, lemma: str, text: str) -> None:
    ...
    queries = [
        ...
        f'MERGE (w)-[:TOKEN_CREATED_BY {{ created_at: datetime("{now}") }}]->(u) WHERE u.user_id = "{user_id}" AND w.word = "{word}" AND w.lemma = "{lemma}" RETURN u'
        f'MERGE (w)-[:BELONGS_TO]->(y) WHERE u.user_id = "{user_id}" AND t.text = "{text}" RETURN w'
    ]
